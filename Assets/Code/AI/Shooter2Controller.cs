...
public class Shooter2Controller : MonoBehaviour, INetworkObject, IDamagableLocal, IPooledObject, IRemoteAgent
{
    [Header("Audio:")]
    ...
    bool TargetIsInRange(Vector3 targetPos)
    {
        ...
        if(distanceToTarget < shootingDistance)
        {
            ...
            RaycastHit hit;
            Ray ray = new Ray(shootingSphereCastPosition.position, dir);
    ...
    void Shoot1(Vector3 pos, Vector3 dir)
    {
    ...
    void UpdateBrain()
    {
        ...
        switch(state)
        {
            ...
            case(ShooterState.Loading):
            {
                if(target)
                {
                    if(loadingTimer > loadingDuration && shotsFired < shotsPerRound)
                    {
                        if(canSendCommands)
                        {
                            LockSendingCommands();
