...
public enum SniperGirlState : byte
{
    Idle,
    Chasing,
    Shooting,
...
public class SniperGirlController : MonoBehaviour, INetworkObject, IDamagableLocal, ILaunchableAirbourne
{
        ...
        switch(command)
        {
            case(NetworkCommand.SetTarget):
            {
                ...
                if(_pv)
                {
                    ...
                    {
                        SetTarget(_pc);
                        SetState(SniperGirlState.Chasing);
                    }
                }
            ...
            case(NetworkCommand.SetState):
            {
                ...
                SetState((SniperGirlState)_state);
            ...
            case(NetworkCommand.Flee):
            {
                ...
                SetState(SniperGirlState.Fleeing);
            ...
            case(NetworkCommand.Shoot):
            {
                ...
                if(state != SniperGirlState.Dead && state != SniperGirlState.Airbourne)
                {
                ...
                SetState(SniperGirlState.Shooting);
            ...
            case(NetworkCommand.LaunchAirborne):
            {
                ...
                SetState(SniperGirlState.Airbourne);
            ...
            case(NetworkCommand.LaunchAirborneUp):
            {
                ...
                if(state != SniperGirlState.Airbourne)
                {
                    Vector3 launchPos = (Vector3)args[0];
    ...
    const float shootMaxDistance = 64;
    const float shootingChasingCooldown = 3.5F;
    float shootingChasingTimer = 0;
    
    const float shootingDuration = 1.1F;//0.5F;
    ...
    void SetState(SniperGirlState _state)
    {
        if(state == SniperGirlState.Dead)
        {
            return;
        ...
        switch(_state)
        {
            case(SniperGirlState.Airbourne):
            {
                if(PhotonNetwork.IsMasterClient)
                {
                    DisableRemoteAgent();
            ...
            case(SniperGirlState.Chasing):
            {
                distanceTravelledRunningSqr = 0;
                brainTimer = 0;
                break;
            }
            case(SniperGirlState.Fleeing):
            {
                distanceTravelledRunningSqr = 0;
                break;
            }
            case(SniperGirlState.Shooting):
            {
                brainTimer = 0;
                break;
            }
    ...
    {
        if(state != SniperGirlState.Dead && damage_taken_timeStamp + 0.15F < Time.time)
        {
    ...
    public SniperGirlState state = SniperGirlState.Idle;
    
    const float PATH_UPDATE_CD = 0.225F;//0.15F;
    float brainTimer = 0;
    ...
    {
        return state == SniperGirlState.Airbourne;
    }
            ...
            case(SniperGirlState.Chasing):
            {
                ...
                else
                {
                    if(canSendCommands)
                    {
                        ...
                        {
                            LockSendingCommands();
                            NetworkObjectsManager.CallNetworkFunction(net_comp.networkId, NetworkCommand.SetState, (byte)SniperGirlState.Idle);   
                        }
                    }
    ...
    const float footStepDistance = 2.5F;
